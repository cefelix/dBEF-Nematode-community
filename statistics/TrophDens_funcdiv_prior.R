...
#for both weeks  
m.Ba_funcdiv_p <- brm(
  bf(Ba_per100g ~ funcdivStd*treatment*week + (1|block/plot),
     hu ~ 1 ),
  data = dat, 
...
#as different orientation of funcdivStd:treatment2:weekW2 and funcdivStd:treatment3:weekW2 -->
#check pairwise with emmeans:
emtrends(m.Ba_funcdiv_p, specs = c("treatment", "week"), var="funcdivStd") %>%
  summary() #CIs overlap

#remove 3way interaction:
m.Ba_funcdiv_p2 <- update(m.Ba_funcdiv_p, 
                          bf(Ba_per100g ~ funcdivStd*treatment + funcdivStd*week + treatment*week + 
                               (1|block/plot),
                             hu ~ 1), 
                          seed = SEED) #2 div
summary(m.Ba_funcdiv_p2, prob =0.9)

emtrends(m.Ba_funcdiv_p2, specs = c("treatment", "week"), var="funcdivStd") %>%
  summary() #CIs overlap

#remove funcdivStd*week
m.Ba_funcdiv_p31 <- update(m.Ba_funcdiv_p, 
                           bf(Ba_per100g ~ funcdivStd*treatment + treatment*week + (1|block/plot),
                              hu ~ 1 + (1|block/plot)), 
                           seed = SEED) #4 div

summary(m.Ba_funcdiv_p31, prob=0.9)
emtrends(m.Ba_funcdiv_p31, specs = c("treatment", "week"), var="funcdivStd") %>%
  summary()

#remove treatment*week 
m.Ba_funcdiv_p32 <- update(m.Ba_funcdiv_p, 
                           bf(Ba_per100g ~ funcdivStd*treatment + funcdivStd*week + (1|block/plot),
                              hu ~ 1), 
                           seed = SEED)
summary(m.Ba_funcdiv_p32, prob=0.9)

emtrends(m.Ba_funcdiv_p32, specs = c("treatment", "week"), var="funcdivStd") %>%
  summary()
...
m.Fu_funcdiv_p2 <- update(m.Fu_funcdiv_p, 
                          bf(Fu_per100g ~ funcdivStd*treatment + funcdivStd*week + treatment*week + 
                               (1|block/plot),
                             hu ~ 1), 
                          seed = SEED)
...
m.Fu_funcdiv_p31 <- update(m.Fu_funcdiv_p, 
                           bf(Fu_per100g ~ funcdivStd*treatment + treatment*week + (1|block/plot),
                              hu ~ 1), 
                           seed = SEED)

#remove treatment*week 
m.Fu_funcdiv_p32 <- update(m.Fu_funcdiv_p, 
                           bf(Fu_per100g ~ funcdivStd*treatment + funcdivStd*week + (1|block/plot),
                              hu ~ 1), 
                           seed = SEED)

#remove funcdivStd*week and treatment*week 
...
m.Pl_funcdiv_p2 <- update(m.Pl_funcdiv_p, 
                          bf(Pl_per100g ~ funcdivStd*treatment + funcdivStd*week + treatment*week + 
                               (1|block/plot),
                             hu ~ 1), 
                          seed = SEED)
...
m.Pl_funcdiv_p31 <- update(m.Pl_funcdiv_p, 
                           bf(Pl_per100g ~ funcdivStd*treatment + treatment*week + (1|block/plot),
                              hu ~ 1), 
                           seed = SEED)

#remove treatment*week 
m.Pl_funcdiv_p32 <- update(m.Pl_funcdiv_p, 
                           bf(Pl_per100g ~ funcdivStd*treatment + funcdivStd*week + (1|block/plot),
                              hu ~ 1), 
                           seed = SEED)
...
m.Pr_funcdiv_p2 <- update(m.Pr_funcdiv_p, 
                          bf(Pr_per100g ~ funcdivStd*treatment + funcdivStd*week + treatment*week + 
                               (1|block/plot),
                             hu ~ funcdivStd + treatment + week + (1|block/plot)), 
                          seed = SEED)
...
m.Pr_funcdiv_p31 <- update(m.Pr_funcdiv_p, 
                           bf(Pr_per100g ~ funcdivStd*treatment + treatment*week + (1|block/plot),
                              hu ~ funcdivStd + treatment + week + (1|block/plot)), 
                           seed = SEED)

#remove treatment*week 
m.Pr_funcdiv_p32 <- update(m.Pr_funcdiv_p, 
                           bf(Pr_per100g ~ funcdivStd*treatment + funcdivStd*week + (1|block/plot),
                              hu ~ funcdivStd + treatment + week + (1|block/plot)), 
                           seed = SEED)
...
m.Pr_funcdiv_p6 <- update(m.Pr_funcdiv_p, 
                          ...
                          seed = SEED)
summary(m.Pr_funcdiv_p6, prob = 0.9)
#remove hu~treatment
...
pp_check(m.Pr_funcdiv_p7, ndraws = 100)
...
pp_check(m.Pr_funcdiv_p8, ndraws = 100)
...
#for both weeks  
m.Om_funcdiv_p <- brm(
  bf(Om_per100g ~ funcdivStd*treatment*week + (1|block/plot),
     hu ~ funcdivStd + treatment + week + (1|block/plot)),
  data = dat, 
  prior = beta_coeff_priors,
  family = hurdle_lognormal,
...
m.Om_funcdiv_p2 <- update(m.Om_funcdiv_p, 
                          bf(Om_per100g ~ funcdivStd*treatment + funcdivStd*week + treatment*week + 
                               (1|block/plot),
                             hu ~ funcdivStd + treatment + week + (1|block/plot)), 
                          seed = SEED)
...
m.Om_funcdiv_p31 <- update(m.Om_funcdiv_p, 
                           bf(Om_per100g ~ funcdivStd*treatment + treatment*week + (1|block/plot),
                              hu ~ funcdivStd + treatment + week + (1|block/plot)), 
                           seed = SEED)

#remove treatment*week 
m.Om_funcdiv_p32 <- update(m.Om_funcdiv_p, 
                           bf(Om_per100g ~ funcdivStd*treatment + funcdivStd*week + (1|block/plot),
                              hu ~ funcdivStd + treatment + week + (1|block/plot)), 
                           seed = SEED)
...
m.Om_funcdiv_p6 <- update(m.Om_funcdiv_p, 
                          bf(Om_per100g ~ funcdivStd*treatment + (1|block/plot),
                             hu ~ funcdivStd + week + (1|block/plot)), 
                          seed = SEED)
#remove hu~funcdiv
...
pp_check(m.Om_funcdiv_p7, ndraws=100)+
  xlim(0,150)
